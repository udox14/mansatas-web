...
export default function GtkPreview() {
  ...
  return (
    <section id="gtk" className="py-24 bg-slate-950 overflow-hidden relative">
      <div className="absolute top-1/3 -left-1/4 w-[500px] h-[500px] bg-primary-600/10 rounded-full blur-[120px] pointer-events-none" />
        ...
        <div className="mb-16">
          ...
          >
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-heading font-black text-white tracking-tighter max-w-2xl">
              Tenaga Pendidik <span className="text-primary-500">Berdedikasi</span>
            </h2>
            <Link
              href="/gtk"
          ...
          {loading
            ...
            : gtkList.map((item, i) => (
                <motion.div
                  ...
                  className="group relative aspect-[3/4] rounded-2xl overflow-hidden bg-slate-900 border border-white/5 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary-500/20"
                >
                  {/* Photo */}
                  <div className="absolute inset-0">
                    {item.image_url ? (
                       <img
                         ...
                         alt={item.name}
                         className="w-full h-full object-cover grayscale-0 md:grayscale opacity-100 md:opacity-70 md:group-hover:opacity-100 md:group-hover:grayscale-0 transition-all duration-700 scale-100 md:group-hover:scale-110"
                       />
                    ) : (
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                    ...
                    <p className="text-[10px] text-slate-400 line-clamp-1 font-medium tracking-wider uppercase">
                      {item.subject || '-'}
                    </p>
                  </div>
                </motion.div>
